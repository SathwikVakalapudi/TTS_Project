...
def generate_formats(text: str, tts, proc):
    ...
    return hq_bytes, ulaw_bytes, reconverted_bytes, duration_s, gen_time, rtf
...
if generate:
    ...
    with st.spinner(f"Generating on {tts.device}… (first run may take 10–30 s)"):
        hq_bytes, ulaw_bytes, reconv_bytes, duration, gen_time, rtf = generate_formats(
            text, tts, proc
        )
    ...
    m4.metric("Device", tts.device.upper())
